        ...
        if "🔐" in auth_mode:
            ...
            with st.expander("🔑 Esqueceu sua senha?"):
                ...
                if st.button("Solicitar Redefinição"):
                    if reset_email:
                        result = make_xano_request(f'/reset/request-reset-link?email={reset_email}', method='GET')
                        if result:
                ...
                if submitted_s:
                    if name_s and email_s and password_s:
                        if len(password_s) < 8:
                            ...
                            signup_data = {"name": name_s, "email": email_s, "password": password_s}
                            response_data = make_xano_request('/auth/signup', method='POST', data=signup_data)
                            if response_data and response_data.get('authToken'):
                                st.session_state['auth_token'] = response_data.get('authToken')
                                st.success("🎉 Conta criada com sucesso!")
                                st.rerun()
                            # A mensagem de erro (ex: email já existe) é tratada dentro de make_xano_request
...
full_name = user_data.get('name') or 'Estudante'
user_name = full_name.split()[0] if full_name.strip() else 'Estudante'
hour = now.hour
